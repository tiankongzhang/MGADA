...
from fcos_core.engine.trainer_detector import do_train
...
from fcos_core.modeling.discriminator import FCOSDiscriminator, FCOSDiscriminator_CA, FCOSDiscriminator_CM
...
def train(cfg, local_rank, distributed):
    ...
    #gen box
    genbox = build_boxgen(cfg, backbone.out_channels).to(device)
    #gen new feature
    genfeature = build_featuregen(cfg, backbone.out_channels).to(device)
    #fcos head
    fcos = build_rpn(cfg, backbone.out_channels).to(device)

    if cfg.MODEL.INS.USE_DIS_GLOBAL:
        if cfg.MODEL.INS.USE_DIS_P7:
            d_dis_P7 = FCOSDiscriminator(
                num_convs=cfg.MODEL.INS.DIS_P7_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.INS.GRL_WEIGHT_P7,
                grl_applied_domain=cfg.MODEL.INS.GRL_APPLIED_DOMAIN).to(device)
        if cfg.MODEL.INS.USE_DIS_P6:
            d_dis_P6 = FCOSDiscriminator(
                num_convs=cfg.MODEL.INS.DIS_P6_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.INS.GRL_WEIGHT_P6,
                grl_applied_domain=cfg.MODEL.INS.GRL_APPLIED_DOMAIN).to(device)
        if cfg.MODEL.INS.USE_DIS_P5:
            d_dis_P5 = FCOSDiscriminator(
                num_convs=cfg.MODEL.INS.DIS_P5_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.INS.GRL_WEIGHT_P5,
                grl_applied_domain=cfg.MODEL.INS.GRL_APPLIED_DOMAIN).to(device)
        if cfg.MODEL.INS.USE_DIS_P4:
            d_dis_P4 = FCOSDiscriminator(
                num_convs=cfg.MODEL.INS.DIS_P4_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.INS.GRL_WEIGHT_P4,
                grl_applied_domain=cfg.MODEL.INS.GRL_APPLIED_DOMAIN).to(device)
        if cfg.MODEL.INS.USE_DIS_P3:
            d_dis_P3 = FCOSDiscriminator(
                num_convs=cfg.MODEL.INS.DIS_P3_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.INS.GRL_WEIGHT_P3,
                grl_applied_domain=cfg.MODEL.INS.GRL_APPLIED_DOMAIN).to(device)
    ...
    if cfg.MODEL.CM.USE_CM_GLOBAL:
        if cfg.MODEL.CM.USE_DIS_P7:
            cm_P7 = FCOSDiscriminator_CM(
                cfg=cfg,
                num_convs=cfg.MODEL.CM.DIS_P7_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.CM.GRL_WEIGHT_P7,
                grl_applied_domain=cfg.MODEL.CM.GRL_APPLIED_DOMAIN).to(device)
        if cfg.MODEL.CM.USE_DIS_P6:
            cm_P6 = FCOSDiscriminator_CM(
                cfg=cfg,
                num_convs=cfg.MODEL.CM.DIS_P6_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.CM.GRL_WEIGHT_P6,
                grl_applied_domain=cfg.MODEL.CM.GRL_APPLIED_DOMAIN).to(device)
        if cfg.MODEL.CM.USE_DIS_P5:
            cm_P5 = FCOSDiscriminator_CM(
                cfg=cfg,
                num_convs=cfg.MODEL.CM.DIS_P5_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.CM.GRL_WEIGHT_P5,
                grl_applied_domain=cfg.MODEL.CM.GRL_APPLIED_DOMAIN).to(device)
        if cfg.MODEL.CM.USE_DIS_P4:
            cm_P4 = FCOSDiscriminator_CM(
                cfg=cfg,
                num_convs=cfg.MODEL.CM.DIS_P4_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.CM.GRL_WEIGHT_P4,
                grl_applied_domain=cfg.MODEL.CM.GRL_APPLIED_DOMAIN).to(device)
        if cfg.MODEL.CM.USE_DIS_P3:
            cm_P3 = FCOSDiscriminator_CM(
                cfg=cfg,
                num_convs=cfg.MODEL.CM.DIS_P3_NUM_CONVS,
                grad_reverse_lambda=cfg.MODEL.CM.GRL_WEIGHT_P3,
                grl_applied_domain=cfg.MODEL.CM.GRL_APPLIED_DOMAIN).to(device)
    ...
    if cfg.MODEL.USE_SYNCBN:
        ...
        genbox = torch.nn.SyncBatchNorm.convert_sync_batchnorm(genbox)
        genfeature = torch.nn.SyncBatchNorm.convert_sync_batchnorm(genfeature)
        fcos = torch.nn.SyncBatchNorm.convert_sync_batchnorm(fcos)

        if cfg.MODEL.INS.USE_DIS_GLOBAL:
            if cfg.MODEL.INS.USE_DIS_P7:
                d_dis_P7 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(dis_P7)
            if cfg.MODEL.INS.USE_DIS_P6:
                d_dis_P6 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(dis_P6)
            if cfg.MODEL.INS.USE_DIS_P5:
                d_dis_P5 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(dis_P5)
            if cfg.MODEL.INS.USE_DIS_P4:
                d_dis_P4 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(dis_P4)
            if cfg.MODEL.INS.USE_DIS_P3:
                d_dis_P3 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(dis_P3)
        ...
        if cfg.MODEL.CM.USE_CM_GLOBAL:
            if cfg.MODEL.CM.USE_DIS_P7:
                cm_P7 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(cm_P7)
            if cfg.MODEL.CM.USE_DIS_P6:
                cm_P6 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(cm_P6)
            if cfg.MODEL.CM.USE_DIS_P5:
                cm_P5 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(cm_P5)
            if cfg.MODEL.CM.USE_DIS_P4:
                cm_P4 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(cm_P4)
            if cfg.MODEL.CM.USE_DIS_P3:
                cm_P3 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(cm_P3)
    ...
    optimizer["genbox"] = make_optimizer(cfg, genbox, name='genbox')
    optimizer["genfeature"] = make_optimizer(cfg, genfeature, name='genfeature')
    optimizer["fcos"] = make_optimizer(cfg, fcos, name='fcos')

    if cfg.MODEL.INS.USE_DIS_GLOBAL:
        if cfg.MODEL.INS.USE_DIS_P7:
            optimizer["d_dis_P7"] = make_optimizer(cfg, d_dis_P7, name='discriminator')
        if cfg.MODEL.INS.USE_DIS_P6:
            optimizer["d_dis_P6"] = make_optimizer(cfg, d_dis_P6, name='discriminator')
        if cfg.MODEL.INS.USE_DIS_P5:
            optimizer["d_dis_P5"] = make_optimizer(cfg, d_dis_P5, name='discriminator')
        if cfg.MODEL.INS.USE_DIS_P4:
            optimizer["d_dis_P4"] = make_optimizer(cfg, d_dis_P4, name='discriminator')
        if cfg.MODEL.INS.USE_DIS_P3:
            optimizer["d_dis_P3"] = make_optimizer(cfg, d_dis_P3, name='discriminator')
    ...
    if cfg.MODEL.CM.USE_CM_GLOBAL:
        if cfg.MODEL.CM.USE_DIS_P7:
            optimizer["cm_P7"] = make_optimizer(cfg, cm_P7, name='discriminator')
        if cfg.MODEL.CM.USE_DIS_P6:
            optimizer["cm_P6"] = make_optimizer(cfg, cm_P6, name='discriminator')
        if cfg.MODEL.CM.USE_DIS_P5:
            optimizer["cm_P5"] = make_optimizer(cfg, cm_P5, name='discriminator')
        if cfg.MODEL.CM.USE_DIS_P4:
            optimizer["cm_P4"] = make_optimizer(cfg, cm_P4, name='discriminator')
        if cfg.MODEL.CM.USE_DIS_P3:
            optimizer["cm_P3"] = make_optimizer(cfg, cm_P3, name='discriminator')

    scheduler = {}
    scheduler["backbone"] = make_lr_scheduler(cfg, optimizer["backbone"], name='backbone')
    scheduler["genbox"] = make_lr_scheduler(cfg, optimizer["genbox"], name='genbox')
    scheduler["genfeature"] = make_lr_scheduler(cfg, optimizer["genfeature"], name='genfeature')
    scheduler["fcos"] = make_lr_scheduler(cfg, optimizer["fcos"], name='fcos')

    if cfg.MODEL.INS.USE_DIS_GLOBAL:
        if cfg.MODEL.INS.USE_DIS_P7:
            scheduler["d_dis_P7"] = make_lr_scheduler(cfg, optimizer["d_dis_P7"], name='discriminator')
        if cfg.MODEL.INS.USE_DIS_P6:
            scheduler["d_dis_P6"] = make_lr_scheduler(cfg, optimizer["d_dis_P6"], name='discriminator')
        if cfg.MODEL.INS.USE_DIS_P5:
            scheduler["d_dis_P5"] = make_lr_scheduler(cfg, optimizer["d_dis_P5"], name='discriminator')
        if cfg.MODEL.INS.USE_DIS_P4:
            scheduler["d_dis_P4"] = make_lr_scheduler(cfg, optimizer["d_dis_P4"], name='discriminator')
        if cfg.MODEL.INS.USE_DIS_P3:
            scheduler["d_dis_P3"] = make_lr_scheduler(cfg, optimizer["d_dis_P3"], name='discriminator')
    ...
    if cfg.MODEL.CM.USE_CM_GLOBAL:
        if cfg.MODEL.CM.USE_DIS_P7:
            scheduler["cm_P7"] = make_lr_scheduler(cfg, optimizer["cm_P7"], name='discriminator')
        if cfg.MODEL.CM.USE_DIS_P6:
            scheduler["cm_P6"] = make_lr_scheduler(cfg, optimizer["cm_P6"], name='discriminator')
        if cfg.MODEL.CM.USE_DIS_P5:
            scheduler["cm_P5"] = make_lr_scheduler(cfg, optimizer["cm_P5"], name='discriminator')
        if cfg.MODEL.CM.USE_DIS_P4:
            scheduler["cm_P4"] = make_lr_scheduler(cfg, optimizer["cm_P4"], name='discriminator')
        if cfg.MODEL.CM.USE_DIS_P3:
            scheduler["cm_P3"] = make_lr_scheduler(cfg, optimizer["cm_P3"], name='discriminator')
    ...
    if distributed:
        ...
        genbox = torch.nn.parallel.DistributedDataParallel(
            genbox, device_ids=[local_rank], output_device=local_rank,
            # this should be removed if we update BatchNorm stats
            broadcast_buffers=False
        )

        genfeature = torch.nn.parallel.DistributedDataParallel(
            genfeature, device_ids=[local_rank], output_device=local_rank,
            # this should be removed if we update BatchNorm stats
            broadcast_buffers=False
        )
        ...
        if cfg.MODEL.INS.USE_DIS_GLOBAL:
            if cfg.MODEL.INS.USE_DIS_P7:
                d_dis_P7 = torch.nn.parallel.DistributedDataParallel(
                    d_dis_P7, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
            if cfg.MODEL.INS.USE_DIS_P6:
                d_dis_P6 = torch.nn.parallel.DistributedDataParallel(
                    d_dis_P6, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
            if cfg.MODEL.INS.USE_DIS_P5:
                d_dis_P5 = torch.nn.parallel.DistributedDataParallel(
                    d_dis_P5, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
            if cfg.MODEL.INS.USE_DIS_P4:
                d_dis_P4 = torch.nn.parallel.DistributedDataParallel(
                    d_dis_P4, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
            if cfg.MODEL.INS.USE_DIS_P3:
                d_dis_P3 = torch.nn.parallel.DistributedDataParallel(
                    d_dis_P3, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
        ...
        if cfg.MODEL.CM.USE_CM_GLOBAL:
            if cfg.MODEL.CM.USE_DIS_P7:
                cm_P7 = torch.nn.parallel.DistributedDataParallel(
                    cm_P7, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
            if cfg.MODEL.CM.USE_DIS_P6:
                cm_P6 = torch.nn.parallel.DistributedDataParallel(
                    cm_P6, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
            if cfg.MODEL.CM.USE_DIS_P5:
                cm_P5 = torch.nn.parallel.DistributedDataParallel(
                    cm_P5, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
            if cfg.MODEL.CM.USE_DIS_P4:
                cm_P4 = torch.nn.parallel.DistributedDataParallel(
                    cm_P4, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
            if cfg.MODEL.CM.USE_DIS_P3:
                cm_P3 = torch.nn.parallel.DistributedDataParallel(
                    cm_P3, device_ids=[local_rank], output_device=local_rank,
                    # this should be removed if we update BatchNorm stats
                    broadcast_buffers=False
                )
    ...
    model["genbox"] = genbox
    model["genfeature"] = genfeature
    model["fcos"] = fcos

    if cfg.MODEL.INS.USE_DIS_GLOBAL:
        if cfg.MODEL.INS.USE_DIS_P7:
            model["d_dis_P7"] = d_dis_P7
        if cfg.MODEL.INS.USE_DIS_P6:
            model["d_dis_P6"] = d_dis_P6
        if cfg.MODEL.INS.USE_DIS_P5:
            model["d_dis_P5"] = d_dis_P5
        if cfg.MODEL.INS.USE_DIS_P4:
            model["d_dis_P4"] = d_dis_P4
        if cfg.MODEL.INS.USE_DIS_P3:
            model["d_dis_P3"] = d_dis_P3
    ...
    arguments["use_dis_detect_gl"] = cfg.MODEL.INS.USE_DIS_GLOBAL
    arguments["dt_dis_lambda"] = cfg.MODEL.INS.DT_DIS_LAMBDA

    arguments["use_cm_global"] = cfg.MODEL.CM.USE_CM_GLOBAL
    arguments["ga_cm_lambda"] = cfg.MODEL.CM.GL_CM_LAMBDA
    ...
    print('data sample:', cfg.MODEL.ISSAMPLE)
    data_loader = {}
