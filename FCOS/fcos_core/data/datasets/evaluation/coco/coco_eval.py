...
def evaluate_predictions_on_coco(
    ...
    #compute ap of classes
    aind = [i for i, aRng in enumerate(coco_eval.eval['params'].areaRngLbl) if aRng == 'all']
    mind = [i for i, mDet in enumerate(coco_eval.eval['params'].maxDets) if mDet == 100]
    s = coco_eval.eval['precision']
    #print(s.shape, coco_eval.eval['params'].catIds, coco_eval.eval['params'].recThrs)
    # IoU
    iouThr = 0.5
    if iouThr is not None:
        t = np.where(iouThr == coco_eval.eval['params'].iouThrs)[0]
        s = s[t]
    s = s[:,:,:,aind,mind]
    
    for idx in range(s.shape[2]):
        sl = s[:,:,idx]
        print(idx, ':', np.mean(sl[sl>-1]))
    
    #print(coco_eval.stats)
    #print(coco_eval.eval.keys())
    return coco_eval
