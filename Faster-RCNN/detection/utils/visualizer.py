...
class VisImage:
    ...
    def get_image(self):
        ...
        #print(len(img.shape), img.shape, rgb.shape, alpha.shape)
        if len(img.shape)==2:
            nimg = np.zeros((img.shape[0], img.shape[1], 3), dtype=img.dtype)
            nimg[:,:,0] = img
            nimg[:,:,1] = img
            nimg[:,:,2] = img
            #nimg = np.array(, dtype=img.dtype)
            img = nimg
            #print(nimg.shape)
...
class Visualizer:
    def __init__(self, img_rgb, metadata, scale=1.0, instance_mode=ColorMode.IMAGE):
        ...
        '''
        self._default_font_size = 1.25*max(
            np.sqrt(self.output.height * self.output.width) // 90, 10 // scale
        )
