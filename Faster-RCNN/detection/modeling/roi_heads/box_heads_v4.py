...
class VGG16BoxPredictor(nn.Module):
    ...
    def forward(self, box_features):
        ...
        #mbox_features = ebox_features.view(ebox_features.size(0), -1)
        mbox_features = self.classifier(ebox_features)
        #mbox_features_reg = self.classifier(ebox_features)
        #print('--02:',box_features.size())
        #print(mbox_features_cls.size(),mbox_features_reg.size())
        mbox_features = torch.mean(mbox_features, dim=(2, 3))

        class_logits = self.cls_score(mbox_features)
        box_regression = self.bbox_pred(mbox_features)
